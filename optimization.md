...
**L-BFGS**:
限制内存的BFGS介于BFGS和共轭梯度之间: 在非常高的维度 (> 250) 计算和翻转的Hessian矩阵的成本非常高。L-BFGS保留了低秩的版本。此外，scipy版本, [scipy.optimize.fmin_l_bfgs_b()][14], 包含箱边界:
...
- $\tau_k=\begin{cases}
1, &g_k^\top B_kg_k\leq0\cr \min\{\frac{||g_k||^3}{\Delta kg_k^\top B_kg_k},1\}, &otherwise
\end{cases}$
...
由于$\mathbf x_k=(1,1)^\top$，则$g_k=\nabla f(\mathbf x_k)=\left[
\begin{matrix}
4x_1^3+2x_1\\2x_2
\end{matrix}
\right]$，从而得到$g_k=(6,2)^\top$。
同理可得：
...
由于$||s_k^{\hat N}||>\Delta_k$，故取双折线步长为$s_k=s_k^c+\lambda(s_k^{\hat N}-s_k^c),\lambda \in (0,1)$，使得$|s_k^{\hat N}||=\Delta_k$。
解二次方程：
...
- 用脚标的集合表示哪些到达边界约束。
    - $x^*$处的积极集：$I(x^*)=\{i|c_i(x^*)=0\}\Longleftrightarrow \exists \lambda_0,\dots,\lambda_n\geq0  s.t. \lambda_0\nabla f(x^*)-\sum_{i=1}^m \lambda_i^* \nabla_ic_i(x^*)=0$ 
    - 如果$\nabla c_i(x^*)(i\in f(x^*))$线性无关，则存在向量$\mathbf \lambda^*$，使得$\nabla f(x^*)-\sum_{i=1}^m \lambda_i^* \nabla_ic_i(x^*)=0,\lambda_i^*\geq0对偶变量,\lambda_i^*c_i(x^*)=0互补松弛条件$
...
\end{cases}无解\Longleftrightarrow F_{x^*}\cap D_{x^*}=\phi
$$
...
[Anaconda][17]
  ...
  [1]: http://www.scipy-lectures.org/_images/plot_convex_1.png
  [2]: http://www.scipy-lectures.org/_images/plot_convex_2.png
  [3]: http://www.scipy-lectures.org/_images/plot_smooth_1.png
  [4]: http://www.scipy-lectures.org/_images/plot_smooth_2.png
  [5]: https://upload.wikimedia.org/wikipedia/commons/thumb/5/5d/Inequality_constraint_diagram.svg/440px-Inequality_constraint_diagram.svg.png
  [6]: http://www.scipy-lectures.org/_images/plot_gradient_descent_5.png
  [7]: http://www.scipy-lectures.org/_images/plot_gradient_descent_105.png
  ...
  [13]: https://upload.wikimedia.org/wikipedia/commons/thumb/e/e0/NewtonIteration_Ani.gif/300px-NewtonIteration_Ani.gif
  [14]: http://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.fmin_l_bfgs_b.html#scipy.optimize.fmin_l_bfgs_b
  [15]: http://www.scipy-lectures.org/_images/plot_constraints_1.png
